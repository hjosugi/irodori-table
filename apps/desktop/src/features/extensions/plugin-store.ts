...
export const bundledPluginStoreIndex: PluginStoreIndex = {
  ...
  source: "bundled",
  extensions: [
    connector("irodori.duckdb", "DuckDB Connector", ["duckdb", "motherduck"]),
    ...
    connector("irodori.kv-store", "KV Store Connector Pack", ["redis", "kvStore", "dynamodb"]),
    connector("irodori.mongodb", "MongoDB Connector", ["mongodb"]),
    connector("irodori.clickhouse", "ClickHouse Connector", ["clickhouse"]),
    connector("irodori.cassandra", "Cassandra / ScyllaDB Connector", [
      "cassandra",
      "scylladb",
    ]),
    connector("irodori.neo4j", "Neo4j / Memgraph Connector", ["neo4j", "memgraph"]),
    ...
    connector("irodori.iceberg", "Iceberg / S3 Tables Connector", [
      "iceberg",
      "athena",
      "s3Tables",
    ]),
    connector("irodori.object-store", "Object Store Connector", [
      "objectStore",
      "deltaLake",
      "hudi",
    ]),
